...
const Icon = ({ d, size = 20, stroke = "currentColor", strokeWidth = 1.8, fill = "none" }: any) => (
  <svg 
    width={size} 
    height={size} 
    viewBox="0 0 24 24" 
...
const menuItems = [
  { path: "/dashboard",        icon: "home",      text: "Início" },
  { path: "/documents",        icon: "docs",      text: "Documentos" },
  { path: "/documents/new",    icon: "upload",    text: "Upload" },
  { path: "/medical-record",   icon: "ficha",     text: "Ficha Médica" },
  { path: "/reports",          icon: "relatorio", text: "Relatório" },
];
...
const Nav = () => {
  ...
  // Se estiver na tela de login, criar-conta ou landing page, não exibe o Nav logado
  if (pathname === "/entrar" || pathname === "/cadastro" || pathname === "/") {
    return null;
  }
    ...
    <>
        ...
        <div className="sm-top-nav__container">
          ...
          <div className="sm-top-nav__menu">
            {menuItems.map(item => {
              ...
              return (
                <Link key={item.path} href={item.path} className={`sm-top-nav__item${active ? " active" : ""}`}>
                  <Icon d={ICONS[item.icon as keyof typeof ICONS]} size={20} />
                  <span>{item.text}</span>
                </Link>
              );
            })}
      ...
      <nav className="sm-bottom-nav">
        {menuItems.map(item => {
          const active = pathname === item.path;
          return (
            <Link key={item.path} href={item.path} className={`sm-bottom-nav__item${active ? " active" : ""}`}>
              <Icon d={ICONS[item.icon as keyof typeof ICONS]} size={22} />
              <span>{item.text}</span>
            </Link>
          );
        })}
        <Link href="/profile" className={`sm-bottom-nav__item${pathname === "/profile" ? " active" : ""}`}>
